...
val rddCapra = sc.textFile("hdfs:/user/gpasini/dataset/capra")
val rddDC = sc.textFile("hdfs:/user/gpasini/dataset/divinacommedia")
...
val rddL = sc.
   textFile("hdfs:/user/gpasini/dataset/capra").
   flatMap( x => x.split(" ") ).
   map(x => (x,1)).
   reduceByKey((x,y)=>x+y)
rddL.toDebugString